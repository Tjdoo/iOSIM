...
/*!
  *  @protocol   ChatTransDataPTC
  *  @brief   MobileIMSDK 的通用数据通信消息的回调事件接口（如：收到聊天数据事件通知、服务端返回的错误信息事件通知等）。
  *  @discussion   实现此接口后，通过 @link [ClientCoreSDK setChatTransDataEvent:] @/link 方法设置，可实现回调事件的通知和处理。
  */
@protocol ChatTransDataPTC <NSObject>

/*!
  *  @brief   收到普通消息的回调事件通知。
  *  @discussion  应用层可以将此消息进一步按自已的 IM 协议进行定义，从而实现完整的即时通信软件逻辑。
  *
  *  @param   fingerPrintOfProtocal   当该消息需要 QoS 支持时，本回调参数为该消息的特征指纹码，否则为 null
  *  @param   userid   消息的发送者 id（例如：规定发送者 id =“0” 即表示是由服务端主动发过的，否则表示的是其它客户端发过来的消息）
